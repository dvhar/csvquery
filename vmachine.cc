...
PRINTHTML_:
	if (numLimitedPrinted < result->rowlimit){
		iTemp1 = 0;	
		string hbuf = "<tr>";
		do {
			hbuf += "<td>";
			torow[iTemp1].appendToHtmlBuffer(hbuf);
			hbuf += "</td>";
...
DIST_NOALLOC_:
	{
		auto treeArray = distinctDatArray(torow, &q->datArrayLess);
		if (groupDistinct.insert(treeArray).second){
			ip++;
		} else {
			ip = op->p1;
		}
...
FUNC_BASE64_ENCODE_:
	ifnotnull {
		...
		base64_encode((u8*)stk0.u.s, (u8*) newstring, (int)stk0.z, 0);
		newstring[newsize] = 0;
		stk0.freedat();
...
FUNC_BASE64_DECODE_:
	ifnotnull {
		...
		stk0.z = base64_decode((u8*)stk0.u.s, (u8*)newstring, (int)stk0.z);
