...
JOINSET_EQ_:
	...
	pop();
	nexti();
JOINSET_CROSS_: //TODO: optimize cross joins
	{
		auto& vpvector = files[op->p1]->joinValpos[op->p2];
		joinSetStack.emplace_front();
		for (auto& vp : vpvector)
			joinSetStack.front().insert(vp.pos);
	}
	pop();
	nexti();
AND_SET_:
